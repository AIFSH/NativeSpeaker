        ...
        for d in range(len(mp.param["band"]), 0, -1):
            ...
            if d == len(mp.param["band"]):  # high-end band
                X_wave[d], _ = librosa.load(
                    path=mix_path, sr=bp["sr"], mono=False, dtype=np.float32, res_type=bp["res_type"]
                )
...
def spectrogram_to_wave(spec, hop_length, mid_side, mid_side_b2, reverse):
    ...
    wave_left = librosa.istft(stft_matrix=spec_left, hop_length=hop_length)
    wave_right = librosa.istft(stft_matrix=spec_right, hop_length=hop_length)
...
def istft(spec, hl):
    spec_left = np.asfortranarray(spec[0])
    spec_right = np.asfortranarray(spec[1])
    wave_left = librosa.istft(stft_matrix=spec_left, hop_length=hl)
    wave_right = librosa.istft(stft_matrix=spec_right, hop_length=hl)
